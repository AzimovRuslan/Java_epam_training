package by.gsu.epamlab;

public class Constants {
    public static String DELIMITER = ";";
    public static String FILE_NOT_FOUND = "file not found";
    public static String TOTAL_COST = "total cost ";
    public static String BYN = " BYN";
    public static String ELEMENT_NOT_FOUND = "element not found";
    public static String EMPTY_MAP = "empty map";
    public static String MAP_DELIMITER = "=>";
    public static String TOTAL_COST_FOR_DAY = "total cost for ";
    public static String SPACE = " ";
    public static String CARRYOVER = "\n";
    public static String MEAT = "meat";
    public static String FIRST_PURCHASE_MAP = "first purchase map";
    public static String LAST_PURCHASE_MAP = "last purchase map";
    public static String DAY_PURCHASE_MAP = "day purchase map";
    public static int FIRST_ELEMENT = 0;
    public static int SECOND_ELEMENT = 1;
    public static int THIRD_ELEMENT = 2;
    public static String ALL_PURCHASES_ON_MONDAY = "all purchases on monday";
    public static String THE_LAST_WEEKDAY_FOR_BREAD = "the last weekday for bread with price";
    public static String THE_FIRST_WEEKDAY_FOR_BREAD = "the first weekday for bread with price";


}
