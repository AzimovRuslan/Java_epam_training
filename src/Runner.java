...
public class Runner {
    public static void main(String[] args) {
        ...
        //3
        System.out.println(Constants.PASSED_COUNT + trialList.stream().filter(Trial::isPassed).count());

        //4
        trialList.sort(Comparator.comparingInt(sumMark));

        //5
        trialList.stream().mapToInt(sumMark).forEach(System.out::println);

        //6
        List<Trial> failedTrials = trialList.stream().filter(trial -> !trial.isPassed()).map(Trial::getCopy).peek(Trial::clearMark).peek(System.out::println).collect(Collectors.toList());
        System.out.println(failedTrials.stream().noneMatch(Trial::isPassed));

        //7
        int[] arrayMarks = trialList.stream().mapToInt(sumMark).toArray();
